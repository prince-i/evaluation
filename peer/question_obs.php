...

{
    ...
    $user_id=$_SESSION['user_id'];
    $prof_id2 = $_REQUEST['prof_id'];
    $comment = $_POST['comment'];
	
    $createCommentQuery = "INSERT INTO 
                                `obs_com` (prof_id, com_ans) 
                            VALUES 
                                ($prof_id2, '$comment')";
	...
	$conn = mysqli_query($connection,"select * from ques_obs")or die ("query 2 incorrect.......");

	while(list($question_id,$ques_eval,$questionnaire) = mysqli_fetch_array($conn))
	{
		$evl_val=$_POST["radio$question_id"];
		mysqli_query($connection,"INSERT INTO `evaluation_obs`(`prof_id`,`prof_id2`,`ques_id`, `ques_ans`) VALUES ('$user_id','$prof_id2','$question_id','$evl_val')") or die (mysqli_error($connection));
...
<form action="question_obs.php" name="form" method="post" class="form-horizontal">
	     <!-- Begin Page Content -->
                <div class="container-fluid">


<h1 class="h2 mb-2 text-gray-800">CLASSROOM OBS. EVALUATION</h1>
...
<?php 
$conn=mysqli_query($connection,"select id,module, survey, deleted_at from survey where module='CLASSROOM OBSERVATION' AND deleted_at='OPEN'")or die ("query 2 incorrect.......");
...
<p>Please click the circle of the most objectively represents his/her professional and personal characteristics. Use the rating scale below:</p>
<p>1 - poor (P)&emsp;
...
<input type="hidden" name="prof_id2" value="<?php echo $_REQUEST['prof_id2']?>">
...
<a class="btn btn-danger" href="classroomobs.php">CANCEL</a> 
<hr><hr>
